...
- 後台「題庫管理」可新增與編輯四選一題目；新增題目一律未啟用，勾選「遊戲啟用」才會加入下一場挑戰。
- 原有五題標示「預設」，初始維持啟用；自訂題目標示「自訂」。題库為全站共用，每場使用全部勾選題目，上限 50 題。
- 「修改活動」可分別勾選採茶、知識卡與問答，至少保留一項。只保留知識卡時記錄完成，分數為 0。
- 每場建立時在伺服器保存題目、正解、難度與關卡快照。中途改題目或關卡不會改掉既有挑戰的計分依據；舊場次維持原五題規則。
- 關卡與啟用題數會影響總分，建議在新活動開始前設定，避免同場活動不同玩家的分數條件不同。
- 驗證：`node scripts/check-dynamic-game.cjs`、`node scripts/check-scoring.cjs`、`node scripts/check-activity-pause.cjs`。
